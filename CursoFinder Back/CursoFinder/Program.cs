...
builder.Services.AddAuthentication(options =>
{
    ...
    options.DefaultChallengeScheme = JwtBearerDefaults.AuthenticationScheme; // ESSENCIAL para evitar redirecionamento!
})
    ...
    options.AddPolicy(name: MyAllowSpecificOrigins,
        policy =>
        {
            policy.WithOrigins("http://localhost:3000")
                  .AllowAnyMethod()
                  .AllowAnyHeader()
                  .AllowCredentials(); // caso esteja usando cookies (opcional para JWT)
        });
});
...
async Task CriarRoles(IServiceProvider serviceProvider)
{
    ...
    string[] roles = { "Usuario", "AdminFaculdade", "AdminGeral" };
